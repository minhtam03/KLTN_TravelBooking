...
export default function Post({ post }) {
  ...
  return (
    <Card sx={{ height: '100%', borderRadius: '12px' }}>
      ...
      <CardContent>
        <Typography variant="h5" sx={{ fontFamily: 'Mulish', mb: 2 }}>
          <Link to={`/blog/${post._id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            {post.title}
          </Link>
        </Typography>
        <Typography variant="body1" paragraph sx={{ fontFamily: 'Mulish' }}>
          <Link to={`/blog/${post._id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            {post.content.length > 150 ? `${post.content.substring(0, 150)}...` : post.content}
          </Link>
        </Typography>
      ...
      <CardActions>
        <Tooltip title={isLiked ? "Unlike" : "Like"}>
          <IconButton onClick={onLikeBtnClick} color={isLiked ? "error" : "default"}>
            <FavoriteIcon />
            <Typography component="span" sx={{ marginLeft: 0.5, fontFamily: 'Mulish' }}>
              {likeCount}
            </Typography>
          </IconButton>
        </Tooltip>
